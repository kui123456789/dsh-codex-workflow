...
test("parseAlignment accepts aligned and conflict shapes and rejects malformed output", () => {
  assert.deepEqual(parseAlignment(JSON.stringify(DEFAULT_ALIGN)), DEFAULT_ALIGN);
  const parsed = parseAlignment(JSON.stringify(ALIGN_CONFLICT));
  ...
  // Invalid entries are dropped, malformed rows throw.
  const dropped = parseAlignment(JSON.stringify({ aligned: false, conflicts: [{ kind: "bogus", index: 0 }, { kind: "finding", index: -1 }] }));
  assert.equal(dropped.aligned, false);
  assert.equal(dropped.conflicts.length, 0);
  assert.throws(() => parseAlignment('{"conflicts":[]}'), /missing aligned/);
  assert.throws(() => parseAlignment("not json"), SyntaxError);
});
...
test("bridge path: unresolved conflict fails the submission WITHOUT staging a verdict or a fix prompt", async () => {
  ...
  try {
    ...
    gateway.reviewResults = [passVerdict, passVerdict]; // re-normalization outputs (unused for the refusal, but keep queues consistent)
    gateway.alignResults = [ALIGN_CONFLICT, ALIGN_CONFLICT];
    gateway.reconcileReplies = [RAW_RECONCILED_REVIEW];
